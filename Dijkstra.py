...
class SumoNetworkAgentPath:
    ...
    def calculate_weight(sumo_network, current_pos, next_pos, destination_pos):
        ...
        ## print({'current_pos':current_pos,'next_pos':next_pos,'destination_pos':destination_pos,'direction_weight':direction_weight,'safety_weight':safety_weight,'priority_weight':priority_weight,'weight':weight*100})
        return weight
    ...
    def find_best_path(sumo_network, start_point, destination, min_points, interval):
        ...
        with open('path_weights.txt', 'a') as file:
            for _ in range(min_points - 1):

                next_paths = sumo_network.get_next_route_paths(current_point, 1, interval)
                
                best_next_point = None
                best_weight = float('-inf')

                for next_points in next_paths:
                    for next_point in next_points:
                        weight = SumoNetworkAgentPath.calculate_weight(sumo_network, current_point, next_point, destination)
                        if weight > best_weight:
                            best_next_point = next_point
                            best_weight = weight
                file.write(str(best_weight) + '\n')          
    ...
    def rand_path(sumo_network, min_points, destination,interval, segment_map, rng=random):
        ...
        return SumoNetworkAgentPath(route_paths, min_points, interval)
...
def do_gamma(c, car_agents, bike_agents, pedestrian_agents, destroy_list):
    ...
    if len(agents) > 0:
        ...
        for agent in agents:
            ...
            occupied_vertices = c.occupied_vertex
            selected_vertices = c.selected_vertex
            agent.path_data.append({
                    'route_points': get_position(actor),
                    'velocity': get_velocity(agent.actor).length(),
                    'acceleration': agent.actor.get_acceleration()
                })
            ...
            if agent.type_tag == 'Car' or agent.type_tag == 'Bicycle':
                ...
                if is_valid:
                    ...
                    target_position = agent.path.get_position(c.sumo_network, 1)  ## to check
...
def do_control(c, speed_pid_integrals, speed_pid_last_errors, steer_pid_integrals, steer_pid_last_errors, pid_last_update_time):
    ...
    for (actor_id, type_tag, control_velocity, preferred_speed, steer_angle_range) in control_velocities:
        ...
        if type_tag == 'Car' or type_tag == 'Bicycle':
            ...
            if type_tag == 'Bicycle':
                target_speed = np.clip(target_speed, speed - 3.0, min(speed + 2.0, c.args.speed_bike)) 
    ...
    try:
        ...
        while True:
            ...
            if not stop_spawning:
                vehicle_count = len(car_agents) + len(bike_agents) + len(pedestrian_agents)
            ...
            if stop_spawning and len(car_agents) == 0 and len(bike_agents) == 0 and len(pedestrian_agents) == 0:
                break
            # 计算和打印模拟的运行频率
