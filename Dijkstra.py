...
class SumoNetworkAgentPath:
    ...
    # 计算方向权重
    def calc_direction_weight(self, current_pos, next_pos, destination_pos):
        """
        ...
        if isinstance(current_pos, carla.SumoNetworkRoutePoint):
            current_pos = self.sumo_network.get_route_point_position(current_pos)
        if isinstance(next_pos, carla.SumoNetworkRoutePoint):
            next_pos = self.sumo_network.get_route_point_position(next_pos)
        if isinstance(destination_pos, carla.SumoNetworkRoutePoint):
            destination_pos = self.sumo_network.get_route_point_position(destination_pos)
        try:
            # 计算当前位置到目的地的方向
    ...
    def calc_safety_weight(self, next_vertex, occupied_vertices, selected_vertices):
        """
        ...
        if isinstance(next_vertex, carla.SumoNetworkRoutePoint):
           next_vertex = self.sumo_network.get_route_point_position(next_vertex)
        if next_vertex in occupied_vertices:
            return 0.5
        elif next_vertex in selected_vertices:
            return 0.1
        else:
            return 0.9
       
    # 计算转向角度
    def calculate_angle(self, current_pos, next_pos):
        """
    ...
    def calc_priority_weight(self, current_pos, next_pos):
        if isinstance(current_pos, carla.SumoNetworkRoutePoint):
            current_pos = self.sumo_network.get_route_point_position(current_pos)
        if isinstance(next_pos, carla.SumoNetworkRoutePoint):
            next_pos = self.sumo_network.get_route_point_position(next_pos)
        angle = self.calculate_angle(current_pos, next_pos)
    ...
    def choose_destination(sumo_network, current_node, min_points, interval):
        ...
        destination_pos = random.choice(destination_pos)

        return destination_pos
    @staticmethod
    def improved_dijkstra(start, end, min_points, interval, sumo_network, ego_vehicle, other_vehicles):
        ...
        while queue:
            ...
            if current_node == end:
                ...
                return path[::-1]
            neighbors = sumo_network.get_next_route_paths(current_node, min_points - 1, interval)
            print("neighbors",neighbors)
            print("*******")
            # 遍历当前节点的邻居
            for neighbor in neighbors: #获取接下来的路径点 
                print("neighbor",neighbor)
                print("*******")
                current_node = sumo_network.get_route_point_position(current_node)
                neighbor = sumo_network.get_route_point_position(neighbor)
                print("neighbor",neighbor)
                print("*******")
                
                # --- 计算综合权值 ---
                w_dir = SumoNetworkAgentPath.calc_direction_weight(current_node, neighbor,end)
                w_safe = SumoNetworkAgentPath.calc_safety_weight(neighbor, other_vehicles) 
                next_dir = sumo_network.get_next_route_points(neighbor, 2)[0]
                w_pri = SumoNetworkAgentPath.calc_priority_weight(next_dir)
                weight = w_dir * w_safe * w_pri
    ...
    #由dijkstra算法生成代理路径
    def update_path_dijkstra(self, sumo_network, origin, destination, occupied_vertices, selected_vertices, other_vehicles):
        shortest_path = self.get_shortest_path(sumo_network, origin, destination, occupied_vertices, selected_vertices, other_vehicles)
        if shortest_path:
            self.route_points = shortest_path
            path_data = {
                'route_points': [sumo_network.get_route_point_position(p) for p in shortest_path],
                'velocity': get_velocity(self.actor).length(),
                'acceleration': self.actor.get_acceleration().length()
            }
...
def do_spawn(c):
    ...
    if spawn_car:
        ...
        for _ in range(SPAWN_DESTROY_REPETITIONS): #SPAWN_DESTROY_REPETITIONS为循环次数
            ...
            # 生成segment_map
            segment_map = c.sumo_network.create_segment_map()
            # spawn_point:生成点，end_point:终点
            spawn_point = segment_map.rand_point()
            # 从SUMO网络中用rand_path生成一条路径
            path = SumoNetworkAgentPath.rand_path(c.sumo_network, PATH_MIN_POINTS, PATH_INTERVAL, segment_map, rng=c.rng)
            # 获取路径的起始位置 0代表索引起始位置
    ...
    if len(agents) > 0:
        ...
        for agent in agents:
            ...
            if agent.type_tag == 'Car' or agent.type_tag == 'Bicycle':
                position = get_position(actor) # 获取代理的位置

                target_position = SumoNetworkAgentPath.choose_destination(c.sumo_network, position, PATH_MIN_POINTS, PATH_INTERVAL) # 选择一个目标位置
                
                if len(agent.path.route_points) <= agent.path.min_points:
                    target_position = agent.path.get_position(c.sumo_network, -1)
                    other_vehicles = [a.actor for a in agents if a.actor.id != actor.id]
                    new_route_points = SumoNetworkAgentPath.improved_dijkstra(position, target_position, PATH_MIN_POINTS, PATH_INTERVAL,
                                                        c.sumo_network, actor, other_vehicles)
                if new_route_points:
                    agent.path.route_points = [position] + new_route_points
                else:
                    agent.selected_vertex = agent.path.route_points[1] if len(agent.path.route_points) > 1 else None
...
def gamma_loop(args):
    path_data_file = open('path_data.csv', 'w', newline='')
        ...
        while True:
            ...
            for agent in car_agents + bike_agents + pedestrian_agents:
                if isinstance(agent.path, SumoNetworkAgentPath):
                    weights = agent.path.get_shortest_path(c.sumo_network, agent.path.route_points[0], agent.path.route_points[-1], [], [], [])
                    ax.plot(weights)
            ax.set_xlabel('Step')
            ax.set_ylabel('Accumulated Weight')
            ax.set_title('Accumulated Weight Change')
            fig.canvas.draw()
            """plt.pause(0.001) """ # 暂停一段时间以更新绘图
            # Download bounds.
