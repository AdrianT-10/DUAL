...
class SumoNetworkAgentPath:
    ...
    @staticmethod
    # 静态方法，在SUMO网络中随机生成代理路径
    def rand_path(sumo_network, min_points, interval, segment_map, rng=random):
        spawn_point = None # 出生点
        route_paths = None # 路径
        while not spawn_point or len(route_paths) < 1: # 如果出生点为空或者路径长度小于1
            spawn_point = segment_map.rand_point() # 随机选择一个点作为起点
            
            spawn_point = sumo_network.get_nearest_route_point(spawn_point) #获取最近的路径点
            
            route_paths = sumo_network.get_next_route_paths(spawn_point, min_points - 1, interval) #获取接下来的路径点 
                   
        return SumoNetworkAgentPath(rng.choice(route_paths), min_points, interval) # 返回随机的一条路径
...
class Context(object):
    def __init__(self, args):
        ...
        self.grid_size = 5  # 每个网格的大小,单位为米
        self.grid_resolution = int((self.bounds_max.x - self.bounds_min.x) / self.grid_size)  # 网格分辨率
        self.fig, self.ax = plt.subplots(figsize=(10, 10))  # 创建图形和坐标轴对象
        
    def get_vehicle_bounding_boxes(self):
        """
        在这个方法中,我们遍历世界中的所有演员
        如果演员是车辆,我们获取其边界框,并将其添加到列表中。
        """
        bounding_boxes = []
        for actor in self.world.get_actors():
            if isinstance(actor, carla.Vehicle):
                bbox = carla.BoundingBox(actor.get_transform().location, actor.bounding_box.extent)
                bounding_boxes.append(bbox)
        return bounding_boxes
    
    # 绘制占用地图
    def plot_occupancy_grid_map(self, bounding_boxes):
        self.ax.clear()  # 清除图形

        # 创建一个空的网格地图
        grid_map = np.zeros((self.grid_resolution, self.grid_resolution))

        # 遍历所有车辆的边界框
        for bounding_box in bounding_boxes:

            # 检查车辆是否在模拟边界内
            if not (self.bounds_min.x <= bounding_box.location.x - bounding_box.extent.x and
                    bounding_box.location.x + bounding_box.extent.x <= self.bounds_max.x and
                    self.bounds_min.y <= bounding_box.location.y - bounding_box.extent.y and
                    bounding_box.location.y + bounding_box.extent.y <= self.bounds_max.y):
                continue  # 如果车辆不在模拟边界内,跳过这个车辆

            # 计算边界框在网格地图中的索引范围
            min_x = max(0, int((bounding_box.location.x - bounding_box.extent.x - self.bounds_min.x) / self.grid_size))
            min_y = max(0, int((bounding_box.location.y - bounding_box.extent.y - self.bounds_min.y) / self.grid_size))
            max_x = min(self.grid_resolution - 1, int((bounding_box.location.x + bounding_box.extent.x - self.bounds_min.x) / self.grid_size))
            max_y = min(self.grid_resolution - 1, int((bounding_box.location.y + bounding_box.extent.y - self.bounds_min.y) / self.grid_size))

            # 计算边界框与网格的相交面积
            for i in range(min_x, max_x + 1):
                for j in range(min_y, max_y + 1):
                    grid_rect_min = carla.Vector3D(i * self.grid_size + self.bounds_min.x, j * self.grid_size + self.bounds_min.y, 0)
                    grid_rect_max = carla.Vector3D((i + 1) * self.grid_size + self.bounds_min.x, (j + 1) * self.grid_size + self.bounds_min.y, 0)

                    # 计算边界框与网格的相交部分
                    intersection_min = carla.Vector3D(
                        max(grid_rect_min.x, bounding_box.location.x - bounding_box.extent.x),
                        max(grid_rect_min.y, bounding_box.location.y - bounding_box.extent.y),
                        0
                    )
                    intersection_max = carla.Vector3D(
                        min(grid_rect_max.x, bounding_box.location.x + bounding_box.extent.x),
                        min(grid_rect_max.y, bounding_box.location.y + bounding_box.extent.y),
                        0
                    )

                    # 计算相交部分的面积
                    intersection_area = max(0, intersection_max.x - intersection_min.x) * max(0, intersection_max.y - intersection_min.y)

                    if intersection_area > 0.8 * self.grid_size ** 2:
                        grid_map[j, i] = 2  # 大部分在方格内,标记为2(红色)
                    elif intersection_area > 0:
                        grid_map[j, i] = max(grid_map[j, i], 1)  # 小部分相交,标记为1(黄色),并确保不会覆盖已有的红色网格

        # 绘制网格线
        for i in range(self.grid_resolution + 1):
            self.ax.plot([self.bounds_min.x + i * self.grid_size, self.bounds_min.x + i * self.grid_size],
                        [self.bounds_min.y, self.bounds_max.y], color='gray', linewidth=0.5)
            self.ax.plot([self.bounds_min.x, self.bounds_max.x],
                        [self.bounds_min.y + i * self.grid_size, self.bounds_min.y + i * self.grid_size],
                        color='gray', linewidth=0.5)

        # 绘制网格地图
        cmap = colors.ListedColormap(['white', 'yellow', 'red'])
        self.ax.imshow(grid_map, cmap=cmap, origin='lower', extent=[self.bounds_min.x, self.bounds_max.x,
                                                                    self.bounds_min.y, self.bounds_max.y], alpha=0.7)
        ...
        self.fig.canvas.flush_events()  # 处理待处理的UI事件
        plt.pause(0.001)
...
def gamma_loop(args):
    try:
        ...
        # 写入CSV文件头
        csv_writer.writerow(['Timestamp', 'Vehicle ID', 'X', 'Y', 'Speed'])
        ...
        while True:
            ...
            if last_bounds_update is None or start - last_bounds_update > 1.0:
                ...
                # 将车辆数据写入CSV文件
                current_time = time.time() - statistics.start_time
                for agent in car_agents:
                    vehicle_id = agent.actor.id
                    x, y = get_position(agent.actor).x, get_position(agent.actor).y
                    speed = get_velocity(agent.actor).length()
                    csv_writer.writerow([current_time, vehicle_id, x, y, speed])
                ...
                bounding_boxes = c.get_vehicle_bounding_boxes()
                
                last_bounds_update = time.time()
                c.plot_occupancy_grid_map(bounding_boxes)
