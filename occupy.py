...
class Context(object):
    def __init__(self, args):
        ...
        self.fig, self.ax = plt.subplots(figsize=(10, 10))# 创建一个新的图形

        
    # 获取车辆的边界框和占用地图      
    def get_vehicle_occupancy(self):
        """
        在这个方法中,我们首先创建一个覆盖整个模拟环境的空占用图。
        然后,我们遍历世界中的所有演员
        如果演员是车辆,我们获取其边界框,并将其添加到占用图中。
        """
        occupancy_map = carla.OccupancyMap(self.bounds_min, self.bounds_max)
        for actor in self.world.get_actors():
            if isinstance(actor, carla.Vehicle):
                bbox = get_vehicle_bounding_box_corners(actor)
                occupancy_map = occupancy_map.union(carla.OccupancyMap(bbox))
        return occupancy_map
    
    # 绘制占用地图
    def plot_occupancy_map(self,occupancy_map,bounding_boxes):

        self.ax.clear() # 清除图形
        # 获取占用地图的多边形
        # 绘制模拟环境的占用地图
        polygons = occupancy_map.get_polygons()
        for polygon in polygons:
            # 将多边形的顶点分离为x和y坐标
            x = [v.x for v in polygon[0]]
            y = [v.y for v in polygon[0]]
            self.ax.fill(x, y, color='gray',alpha=0.5)

        # 绘制车辆、自行车和行人占用图
        for bounding_box in bounding_boxes:
            polygons = bounding_box.get_polygons()
            for polygon in polygons:
                x = [v.x for v in polygon[0]]
                y = [v.y for v in polygon[0]]
    ...
    try:
        ...
        while True:
            ...
            if last_bounds_update is None or start - last_bounds_update > 1.0:
                ...
                last_bounds_update = time.time()
                c.plot_occupancy_map(c.bounds_occupancy, bounding_boxes)
