...
class SumoNetworkAgentPath:
    ...
    def update_path_astar(self, sumo_network, origin, occupied_vertices, selected_vertices, other_vehicles):
        if not self.destination_pos:
            # 如果没有设置目标位置,先随机生成一条路径
            self.route_points = SumoNetworkAgentPath.rand_path(sumo_network, self.min_points, self.interval, sumo_network.create_segment_map(), rng=random).route_points
            self.destination_pos = sumo_network.get_route_point_position(self.route_points[-1])

        shortest_path = self.get_shortest_path_astar(sumo_network, origin, self.destination_pos, occupied_vertices, selected_vertices, other_vehicles)
        if shortest_path:
    ...
    def get_shortest_path_astar(self, sumo_network, origin, destination, occupied_vertices, selected_vertices, other_vehicles):
        def heuristic(route_point):
            a = sumo_network.get_route_point_position(route_point)
            b = destination
            return math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2)

        def route_point_to_key(route_point):
            return (route_point.edge, route_point.lane, route_point.segment, route_point.offset)
        ...
        g_score[route_point_to_key(origin)] = 0
        f_score = defaultdict(lambda: float('inf'))
        f_score[route_point_to_key(origin)] = heuristic(origin)

        while open_set:
            current = heapq.heappop(open_set)[1]
            current = sumo_network.get_route_point_position(current)
            if math.sqrt((current.x - destination.x) ** 2 + (current.y - destination.y) ** 2) < 1.0:
                return reconstruct_path(came_from, current)

            next_points = sumo_network.get_next_route_points(current, self.interval)
            for next_point in next_points:
                tentative_g_score = g_score[route_point_to_key(current)] + sumo_network.get_route_point_position(current).distance(sumo_network.get_route_point_position(next_point))
                if tentative_g_score < g_score[route_point_to_key(next_point)]:
                    came_from[next_point] = current
                    g_score[route_point_to_key(next_point)] = tentative_g_score
                    f_score[route_point_to_key(next_point)] = tentative_g_score + heuristic(next_point)
                    heapq.heappush(open_set, (f_score[route_point_to_key(next_point)], next_point))
        return None
...
def do_spawn(c):
    ...
    if spawn_car:
        ...
        for _ in range(SPAWN_DESTROY_REPETITIONS): #SPAWN_DESTROY_REPETITIONS为循环次数
            ...
            path = SumoNetworkAgentPath.rand_path(c.sumo_network, PATH_MIN_POINTS, PATH_INTERVAL, segment_map, rng=c.rng)            # 获取路径的起始位置 0代表索引起始位置
...
def do_gamma(c, car_agents, bike_agents, pedestrian_agents, destroy_list):
    ...
    if len(agents) > 0:
        ...
        for agent in agents:
            ...
            if agent.type_tag == 'Car' or agent.type_tag == 'Bicycle':
                position = get_position(agent.actor)
                nearest_point = c.sumo_network.get_nearest_route_point(position)
                occupied_vertices = [a.occupied_vertex for a in agents if a != agent]
                selected_vertices = [a.selected_vertex for a in agents if a != agent and a.selected_vertex is not None]

                if not agent.path.destination_pos:
                    segment_map = c.sumo_network.create_segment_map()
                    destination_pos = [element.end for element in segment_map.get_segments()]
                    agent.path.destination_pos = random.choice(destination_pos)

                if not agent.path.update_path_astar(c.sumo_network, nearest_point, occupied_vertices, selected_vertices, agent.other_vehicles):
                    is_valid = False
                else:
    ...
    for agent in next_agents:
        if agent.type_tag == 'Car' or agent.type_tag == 'Bicycle':
            position = get_position(agent.actor)
            target_position = agent.path.get_position(c.sumo_network, 5)
            velocity = (target_position - position).make_unit_vector()
            agent.control_velocity = agent.preferred_speed * velocity
            if is_valid:
                target_position = agent.path.get_position(c.sumo_network, 5)
                velocity = (target_position - position).make_unit_vector()
                agent.control_velocity = agent.preferred_speed * velocity
